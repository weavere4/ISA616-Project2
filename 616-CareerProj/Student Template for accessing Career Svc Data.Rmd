...
Molly Wimer and Elijah Weaver's Business Value Proposition for dealing with geographic tend problems. **Description of Audience and Purpose**
...
```{r}
## NOTES##
# do we want to get rid of the latin honors variable because its already bening represnted by dummy variables
  #   if so use this code.   select(-BBRJ) %>% and replace it with the mutate latin honors
# Not dropping others after I had a conversation with her she said that its a small enough dataset that we dont really need to drop any information incase we need to use it later. i.e internships and department
#
```
